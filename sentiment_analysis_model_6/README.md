...
مقدمه : سه مدل برای طبقه بندی متن. در دو مدل از ترنسفورمر های هیبرید -ساخته شده با کانولوشن یک بعدی و شبکه عصبی بازگشتی gru به جای لایه خطی- استفاده شده. مدل سوم با ترنسفورمر استاندارد طراحی شده. دیتاست استفاده شده، یک دیتاست ۲۰ کلاسه است

---
### 🔴 اشکالات !!
##### 🔺 نوشته های پایین (بعد از خط) در مورد نتایج آموزش های این کد هستند ( قبل از اینکه اشکالات زیر را بفهمم نوشتمشون، ولی پاک نمیکنم تا نسخه اصلی کد با تحلیل نتایجش بمونه !)
##### 🔺در LR-Scheduler وقتی مانیتور val-loss هست باید mode برابر min باشه !! یعنی کنترل نرخ یادگیری در راستای کاهش val-loss انجام بشه !! در حالی که توی کد اشتباها max گذاشته بودم ! همین موضوع تا حدودی کار خرابی میکرد !! البته اینم بگم که اونقدر تاثیر نذاشت، ولی خب دیگه درست نیست، باید اصلاح بشه،نسخه اصلاح شده را قرار میدهیم
##### 🔺 در ضمن اگر batch-size  رو 16 یا 32 میزاشتیم خیلی بهتر بود. سرعت آموزش خیلی بیشتر میشه. البته الان هم که برابر 1 هست غلط نیست، ولی خب بیشتر باشه بهتره. اون موقع که کد میزدم حواسم به این موضوع نبود.
##### 🔺 در واحد های feedforward خروجی دو کانولوشن رو concate کردم و خروجی رو به لایه خطی و lstm دادم. ولی بعد مشخص شد این کار بیشتر از اینکه توی این مسئله کمک کنه نویز داده رو بیشتر میکرده!! در کد اصلاح شده بعدی این مورد برداشته شد. به جاش کانولوشن ها حالت سریالی پیدا کردن و بینشون هم یک ReLU قرار داده شد. کاربرد ReLu اینجا اینه که یکم این تکه کد رو از حالت خطی در بیاره.
##### 🔺 در این کد برای هر بلاک feedforward و Attention یک مدل تعریف کردم. ولی این کار از لحاظ برنامه نویسی جالب نیست. بهتر بود یک کلاس CustomTransformer میداشتیم و بلاک های feedforward و Attention ورودی هاش بودن. اینجوری هر چقدر بخواهیم میتونیم attention , feedforward تعریف کنیم و خیلی راحت سرِ همشون کنیم. در مورد مدل هم همینطور. نیاز نیست برای هر ترنسفورمر جدید یک مدل تعریف کنیم. میتونستیم کلا یک کلاس مدل تعریف کنیم و ترنسفورمر های مورد نظر رو ورودی init بدیم. در کد اصلاح شده بعدی این موارد در نظر گرفته شدن (Modularity).
##### 🔺 نکته ی دیگه ای هم که هست اینه که بلاک feedforward نباید زیاد پیجیدگی داشته باشه. بلاک feedforward بهتره با لایه های خطی و اکتیویشن ها طراحی بشه. ( linear , relu , gelu , conv ) 
---
